...
// credit card payment form - shown after picking "Pay with Credit Card" on AppPayDepositPanel
public class AppPayDepositCreditCard extends JPanel {
    ...
    public AppPayDepositCreditCard(double amount, Runnable onBack, Runnable onSuccess, Account account, Room room, LocalDateTime startTime, LocalDateTime endTime, double finalCost) {
        this.amount = amount;
    ...
    private JPanel buildHeader(Runnable onBack) {
        ...
        JLabel title = new JLabel("Pay with Credit Card");
    ...
    private JComponent buildFormCard() {
        ...
        JPanel cvvCol = new JPanel();
        cvvCol.setOpaque(false);
        cvvCol.setLayout(new BoxLayout(cvvCol, BoxLayout.Y_AXIS));
        cvvCol.add(buildFieldLabel("CVV"));
        cvvCol.add(Box.createVerticalStrut(6));
        cvvCol.add(cvvField);
        ...
        cvvField.getDocument().addDocumentListener(validityListener);
    ...
    private void submitPayment() {
        // TODO: confirm this ordering matches what CreditCardProcessorStrategy expects
        ...
        paymentDetails.add(new String(cvvField.getPassword()).trim());

        boolean successful = paymentController.processPayment(amount, PaymentMethod.CREDITCARD, paymentDetails);
        ...
        if (successful) {
        	...
        	paymentController.storePayment(booking.getBookingId(), this.amount, PaymentType.DEPOSIT, PaymentMethod.CREDITCARD);
            showSuccess();
        } else {
